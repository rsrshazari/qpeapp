...
<!-- Carousel wrapper -->
  <div class="container">
  <?php
$slider=$this->db->query("select * from slider_image where store_id='".$store_data['id']."'")->result();
       $ckh=count($slider);
       //echo $store_data['id'];
if($ckh>0){
 ?>
          ...
          ?>
           <article class="mb-1 mt-1" data-cat="" style="border-radius:20px;height:200px">
            <div class="" style="border-radius: 20px;height:200px">
              <a href="#" >
                <div style="border-radius: 20px;width: 100%;height:100%;background-size:100% 100%;" class="article-image" data-background="<?php echo $imgSrcs;?>" style="background-image: url('<?php echo $imgSrcs;?>');"></div>
              </a>
              </div>
           </article>
              <?php }
      ...
      foreach($product_list_grouped_ordered as $key_main => $value_main) :
       ...
       foreach ($value_main as $key => $value) :
          ...
          if($hide_add_to_cart=='0')
          {
            if($value['attribute_ids']=='')
            {
              $display_add_to_cart =  '<a href="" class="btn  '.$btn_size.' btn-primary add_to_cart" data-attributes="'.$value['attribute_ids'].'" data-product-id="'.$value['id'].'" data-action="add"><i class="fas fa-shopping-cart"></i> '.$cart_lang.'</a>';
            }
            else
            {
