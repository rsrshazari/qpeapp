...
<section class="section section_custom">
  ...
  <div class="section-body">
    <div class="row">
      <div class="col-12">
          <form action="<?php echo base_url("ecommerce/appearance_settings_action"); ?>" method="POST" enctype="multipart/form-data">
              ...
              <div class="row">
                ...
                <div class="col-12 col-md-6">
                  <div class="form-group mb-3">
                    <label>
                      <?php echo $this->lang->line("choose multiple slider image (535 * 200 px) "); ?>
                    </label>
                    <div class="selectgroup d-block">
                      <label class="selectgroup-item">
                        <input type="file" name="slider_image[]" id="files" multiple />
                      </label>
                    </div>
                  </div>
                </div>
                <div class="col-12 col-md-6">
                  <div class="form-group mb-3">
                    <label>
                      <?php echo $this->lang->line("slider image"); ?>
                    </label>
                    <div class="selectgroup d-block">
                      <label class="selectgroup-item">
                      <?php 
            $slider=$this->db->query("select * from slider_image where user_id='".$this->session->userdata('user_id')."' and store_id='".$this->session->userdata('ecommerce_selected_store')."'")->result();
     $ckh=count($slider);
           //echo count($slider);
if($ckh>0){
            foreach($slider as $slider_img){
              ...
              foreach($temp as $val){
              $imgSrcs =base_url('upload/slider_image/'.$val);
                 if ($val!=''){
              ?>
              <span class="pip">
              <img src="<?= $imgSrcs?>" alt="<?= $imgSrcs?>" width="100" height="100">
              <span class="remove" onclick=fn_delete('<?= $slider_img->id?>','<?= $val?>')>Delete image</span>
              </span>
              <?php
                }
              }
            ...
            ?>
            <input type="hidden" name="slider_id" value="<?= @$slider_img->id?>">
                      </label>
                    </div>
                  </div>
                </div>
...
function fn_delete(id,img){
$.ajax({
        url: '<?= base_url()?>ecommerce/delete_slider_image',
        data: {id: id,img:img},
        type: 'POST',
