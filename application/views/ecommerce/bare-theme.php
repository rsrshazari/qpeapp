...
<html lang="en">
    ...
    if(isset($social_analytics_codes))
   	{
   		$store_link = base_url("store/".$social_analytics_codes['slug']);
   		...
   		$first_menu = !empty($store_name_logo) ? '<a class="mr-3 pt-2" href="'.$store_link.'">'.$store_name_logo.'</a>' : '';
   	}
	  		    ...
	  		    <li class="media">
	  		      ...
	  		      <div class="media-body">
	  		         <h4 class="text-white pt-1"><?php echo isset($social_analytics_codes['store_name']) ? $social_analytics_codes['store_name'] : "";?></h4>
								 <h6 class="text-white" style="font-size:0.9rem"><?php echo $social_analytics_codes['store_address'].'-'.$social_analytics_codes['store_city'];?></h6>
	  		      </div>
						</li>
	  		  </ul>
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        $(document).on('click', '#login_btn', function(e) {
            ...
            var otp = $("#otp").val();
            if( otp==""){
                swal("<?php echo $this->lang->line('Error'); ?>", "<?php echo $this->lang->line("Please enter verification code."); ?>", 'error');
                return;
            }
            $("#login_btn").addClass('btn-progress');
            ...
            $.ajax({
              ...
              data:{mobile,otp,store_id,js_user_id},
...
//  $('#loader').show();
var mob1 = $('#login_mobile').val();
var NumberRegex = /^[0-9]*$/;
if(mob1.length == 10){
if(NumberRegex.test(mob1)){
  var mobslice=mob1.slice(mob1.length - 3);
$.ajax({
url: '<?= base_url() ?>ecommerce/get_verification_code',
type: "POST",
data: {
mob: mob1
},
...
//$('#loader').hide();
	...
	swal("<?php echo $this->lang->line('Error'); ?>", "<?php echo $this->lang->line("Plz enter valid mobile no."); ?>", 'error');
	return;
}
}else{
	swal("<?php echo $this->lang->line('Error'); ?>", "<?php echo $this->lang->line("Plz enter valid mobile no."); ?>", 'error');
      ...
      <div class="modal-body text-justify" id="LoginModalBody">
            <form method="POST" action="#" class="needs-validation" novalidate="" _lpchecked="1">
              <div id="mobile_div" class="form-group">
                <label for="login_mobile"><?php echo $this->lang->line("Mobile No");?>*</label>
                <input type="text" class="form-control" id="login_mobile" name="login_mobile"  placeholder="<?php echo $this->lang->line("Mobile Number"); ?>" required autofocus autocomplete="off">
              ...
              <input id="otp" type="text" value=""  class="form-control" name="otp" tabindex="2" required maxlength="6">
  						<div class="invalid-feedback"></div>
						</div>
              <div class="form-group mt-2">
								<button style="display:none" id="login_btn" type="submit" class="btn btn-primary btn-lg btn-block login_btn" tabindex="4">
									<i class="fa fa-sign-in-alt"></i> <?php echo $this->lang->line("Veriy & Continue") ?>
								</button>
