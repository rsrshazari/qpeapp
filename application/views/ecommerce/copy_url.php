<?php
$store_code = array(0=>array("title"=>$this->lang->line("Store Page"),"url"=>base_url("ecommerce/store/."$current_store_data['slug'])));
$category_copy = array();
$order_code = array(0=>array("title"=>$this->lang->line("Buyer's Orders Page"),"url"=>base_url($current_store_data['id'])));
$product_copy = array();
//print_r($current_store_data);
//print_r($order_code);
foreach($category_list as $key => $value)
{
   $store_code[] = array("title"=>$this->lang->line("Store Page")." - ".$this->lang->line("Category")." : ".$value,"url"=>base_url($current_store_data['slug']."?category=".$key));
}
$product_list_assoc = array();                         
foreach ($product_list as $key => $value) 
{
  $product_copy[] = array("title"=>$this->lang->line("Product Page")." : ".$value["product_name"],"url"=>base_url("product/".$value['id']));  
}
?>
