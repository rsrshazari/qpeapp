...
class Home extends CI_Controller
{
    ...
    */
    ...
    public function login() //loads home view page after login (this )
    {
        ...
        $this->form_validation->set_rules('mobileno', 'Mobile Number ', 'required|regex_match[/^[0-9]{10}$/]'); //{10} for 10 digits number
      //  $this->form_validation->set_rules('mobileno', '<b>'.$this->lang->line("Mobile No").'</b>', 'trim|required|valid_email');
        ...
        else
        {
            ...
            $mobileno = strip_tags($this->input->post('mobileno', true));
            $otp = strip_tags($this->input->post('otp', true));
            $vfcode=$this->session->userdata('vfcode');
          ...
          if($otp==$vfcode){
            ...
            else $where['where'] = array('mobile' => $mobileno, "deleted" => "0","status"=>"1");
            ...
            if ($count == 0) {
                $this->session->set_flashdata('login_msg', $this->lang->line("Invalid Credential"));
                redirect(uri_string());
            }
            else
            {
          ...
          $this->session->set_flashdata('login_msg', $this->lang->line("Verification code not matched"));
          redirect(uri_string());
        }
    ...
    public function get_verification_code() {
          ...
          $SixDigitRandomNumber = rand(100000,999999);
           $qry="select * from users where mobile='".$mobileno."' and deleted='0' and status='1'";
...
$this->session->set_userdata($newdata);
  ...
  $msg="<span style='color:red;font-weight:bold'>Mobile no not register</span>";
  $status='0';
  echo $status.'#'.$msg;
}
}
    ...
    public function facebook_login_back()
    {
        ...
        if(is_array($info) && !empty($info) && isset($info["name"]))
        {
            ...
            $this->db->where($sql);
            $where = ['where'=>["deleted" => "0","status"=>"1"]];
            $info = $this->basic->get_data($table, $where, $select = '', $join = '', $limit = '', $start = '', $order_by = '', $group_by = '', $num_rows = 1);
    ...
    public function get_broadcast_tags($media_type='fb')
    {
      ...
      if($media_type=='ig'){
        $dropdown = array("HUMAN_AGENT"=>"HUMAN_AGENT");
        return $dropdown;
      }
    ...
    public function code_genaration()
    {
        ...
        if ($result[0]['num'] == 1) {
            ...
            if ($this->basic->insert_data($table, $info)) {
                ...
                $smtp=1;
                $test_mail=0;
                $this->_mail_sender($from, $to, $subject, $message, $mask, $html,$smtp,'',$test_mail);
            }
        } else {
            echo 0;
        }
