...
class Instagram_reply extends Home
{
    ...
    public function get_account_details()
    {
      if($this->session->userdata('user_type') != 'Admin' && !in_array(279,$this->module_access)) exit;
        ...
        if(!empty($page_info)) {
            ...
            $autoreply_info = $this->basic->get_data('instagram_reply_autoreply', array('where' => array('page_info_table_id' => $page_table_id, 'report_type' => 'post')), array('last_reply_time','auto_comment_reply_count','post_id','id'), '', '', '', 'last_reply_time desc');
                              ...
                              <div class="card-body">
                                <div class="makeScroll">
                                  ...
                                  {     
                                      ...
                                      {
                                        $button .= "<a class='pointer dropdown-item has-icon edit_reply_info_template orange' table_id='".$existing_data_comment[$value['id']]."'><i class='fas fa-edit'></i> {$this->lang->line("edit auto comment")}</a>";
                                      } 
                                      else
    ...
    {  
        ...
        //$response_raw='{"object": "instagram", "entry": [{"id": "17841448207671144", "time": 1627732406, "changes": [{"value": {"media_id": "17914557991754839", "comment_id": "18165710443087192"}, "field": "mentions"}]}]}';
        // $response = json_decode($response_raw,TRUE);
