...
class Ecommerce extends Home {
    ...
    public function __construct() {
        ...
        include_once(APPPATH . "libraries/PaytmKit/lib/encdec_paytm.php");
    ...
    protected function ecommerce_review_comment_exist() {
        if ($this->basic->is_exist("add_ons", array("project_id" => 48)))
            return true;
        return false;
    }
      ...
      public function get_verification_code() {
        ...
        $slice_no=substr($mobileno,-3);
        $SixDigitRandomNumber = rand(100000,999999);
          //$SixDigitRandomNumber = 969496;
        $status='1';
$curl = curl_init();
 curl_setopt_array($curl, array(
   CURLOPT_URL => 'http://sms6.rmlconnect.net:8080/bulksms/bulksms?username=quicqotp&password=quicqo12&type=0&dlr=1&destination='.$mobileno.'&source=QPEIND&message=Dear%20Customer.%20Your%20%20login%20%20OTP%20is%20'.$SixDigitRandomNumber.'.%20-QPEIND&entityid=1001069290050644641&tempid=1007851545181133772%0A',
 ...
 curl_close($curl);
$newdata = array(
                 'cust_vfcode'  => $SixDigitRandomNumber
             );
$this->session->set_userdata($newdata);
$msg="<span style=''>Code send to *******".$slice_no."</span>";
echo $status.'#'.$msg."#".$response;
  ...
  public function login_action()
  {
    ...
    $cust_vfcode=$this->session->userdata('cust_vfcode');
  if($otp==$cust_vfcode){
    $info = $this->basic->get_data("messenger_bot_subscriber",array("where"=>array("phone_number"=>$mobile,"subscriber_type"=>"system","store_id"=>$store_id)));
    if (count($info) == 0) {
    ...
    echo json_encode(array("status"=>"1","message"=>""));
      ...
      echo json_encode(array("status"=>"0","message"=>$this->lang->line("Code does not matched.")));
      exit();
    }
    ...
    public function logout() {
        ...
        $store_slug = $this->input->post("store_slug", true);
        if (strpos($subscriber_id, "sys") !== false && !empty($store_unique_id))
            echo base_url("ecommerce/store/" . $store_slug);
        else
            echo "1";
    }
    ...
    public function notification_settings_action() {
        ...
        foreach ($post as $key => $value) {
            if (!is_array($value) && !in_array($key, $tag_allowed))
        ...
        foreach ($notification_list as $key => $value) {
            ...
            if ($this->session->userdata('user_type') == 'Admin' || in_array(263, $this->module_access))
                if ($notification_email_api_id != '')
                    $notification_array['email'][$value] = isset($email_text[$key]) ? $email_text[$key] : $notification_default['email'];

            if ($this->session->userdata('user_type') == 'Admin' || in_array(264, $this->module_access))
                if ($notification_sms_api_id != '')
                    $notification_array['sms'][$value] = isset($sms_text[$key]) ? $sms_text[$key] : $notification_default['sms'];
        }
    ...
    public function store_list() {
        ...
        if ($store_id == "" && $this->session->userdata("ecommerce_selected_store") == "")
            $store_id = $default_store_id;

        if ($store_id != "")
            $this->session->set_userdata("ecommerce_selected_store", $store_id);
        ...
        if ($to_date == '')
            $to_date = date("Y-m-d");
        if ($from_date == '')
            $from_date = date("Y-m-d", strtotime("$to_date - " . $data_days . " days"));
        ...
        $data['category'] = $this->db->query("SELECT id,name FROM `category` WHERE is_deleted=0")->result();
        $this->_viewcontroller($data);
    }
    ...
    public function store($slug = "") {
        ...
        $where_simple = array("ecommerce_store.slug" => $slug, "ecommerce_store.status" => '1');
        ...
        else if ($product_sort == "sale")
            $product_sort = "sales_count";
        else if ($product_sort == "random")
            $product_sort = "rand()";
        else
            $product_sort = "product_name";

        if ($product_sort != "rand()")
            $product_sort = $product_sort . " " . $product_sort_order;
    ...
    public function product($product_id = 0) {
        ...
        $select = array("ecommerce_product.*", "store_name", "store_unique_id", "slug", "store_logo", "store_favicon", "terms_use_link", "refund_policy_link", "store_locale", "pixel_id", "google_id");
        ...
        if ($this->addon_exist('ecommerce_product_price_variation'))
            $data["attribute_price_map"] = $this->basic->get_data("ecommerce_attribute_product_price", array("where" => array("product_id" => $product_id)));
        else
            $data["attribute_price_map"] = array();
 ...
 echo $output;
    ...
    public function cart($id = 0) {
        ...
        if ($subscriber_id != "")
            $where_simple2['ecommerce_cart.subscriber_id'] = $subscriber_id;
        else
            $where_simple2['ecommerce_cart.user_id'] = $this->user_id;
        $where2 = array('where' => $where_simple2);
    ...
    public function digital_product_orders($store_id = 0, $cart_id = 0) {
        if (!$this->addon_exist("ecommerce_digital_product") && !$this->basic->is_exist("modules", array("id" => 316))) {
        ...
        $login_needed = false;
        if ($subscriber_id != "")
            $where_subs = array("subscriber_type" => "system", "subscribe_id" => $subscriber_id, "store_id" => $store_id);
        else {
            if ($subscriber_id == "")
                $subscriber_id = $this->input->get("subscriber_id", true);
            if ($subscriber_id != "")
                $where_subs = array("subscriber_type!=" => "system", "subscribe_id" => $subscriber_id);
        }
        if ($subscriber_id == '')
            $login_needed = true;
        else {
    ...
    public function download_item($item_id = 0) {
        ...
        if ($item_id == 0 || $item_id == '')
            redirect("access_forbidden", "refresh");
        ...
        if ($item_subscriber_id != $cart_subscriber_id)
            exit;
     ...
     public function order($id=0) // if $id passed means not ajax, it's loading view
  {
    ...
    $select2 = array("ecommerce_cart.*","first_name","last_name","full_name","profile_pic","user_location","email","image_path","phone_number","store_name","slug","store_type","store_email","store_favicon","store_phone","store_logo","store_address","store_zip","store_city","store_country","store_state","store_unique_id","gst_no","tax_percentage","terms_use_link","refund_policy_link","store_locale","pixel_id","google_id");
    $join2 = array('messenger_bot_subscriber'=>"messenger_bot_subscriber.subscribe_id=ecommerce_cart.subscriber_id,left",'ecommerce_store'=>"ecommerce_store.id=ecommerce_cart.store_id,left");
    ...
    $store_gstno = $webhook_data_final["gst_no"];
    $store_tax = $webhook_data_final["tax_percentage"];
...
$gst_percentage="";
 $tax_info = "";
    if($total_tax>0)
      $gst_tax_amount=$total_tax/2;
      $gst_percentage=$store_tax/2;
      $tax_info =
    '<div class="invoice-detail-item">
    <div class="invoice-detail-name">'.$this->lang->line("CGST @") .''.$gst_percentage.'%</div>
    <div class="invoice-detail-value">+'.$currency_left.mec_number_format($gst_tax_amount,$decimal_point,$thousand_comma).$currency_right.'</div>
    </div>
    <div class="invoice-detail-item">
    <div class="invoice-detail-name">'.$this->lang->line("SGST @").''.$gst_percentage.'%</div>
    <div class="invoice-detail-value">+'.$currency_left.mec_number_format($gst_tax_amount,$decimal_point,$thousand_comma).$currency_right.'</div>
    </div>
    ';
    ...
    if($subscriber_id=='')
    {
      ...
      $store_image = ($webhook_data_final['store_logo']!='' && $is_ajax!='1') ? '<div class="col-lg-12 text-center d-print-none-thermal"><a href="'.base_url('store/'.$slug).'"><img style="max-height:50px" src="'.base_url("upload/ecommerce/".$webhook_data_final['store_logo']).'"></a><hr class="m-3 mb-4"></div>':'';
    }
    else
    {
      $wc_buyer_bill_formatted = $wc_buyer_bill;
      $tempu = base_url('store/'.$slug);
    ...
    $order_details_print = '<h5>#'.$order_no.' ('.$order_date2.')</h5><h5>'.$gst_no.'</h5>';
    ...
    public function update_cart_item() {
        ...
        $subscriber_id = $this->session->userdata($product_data[0]['store_id'] . "ecom_session_subscriber_id");
        if ($subscriber_id != "")
            $where_subs = array("subscriber_type" => "system", "subscribe_id" => $subscriber_id, "store_id" => $product_data[0]['store_id']);
        else {
            if ($subscriber_id == "")
        ...
        if (isset($cart_data[0])) { // already have a cart running entry
            ...
            if ($action == "add") {
                $update_cart = [];
                if ($cart_data[0]["buyer_email"] == "")
                    $update_cart['buyer_email'] = $buyer_email;
                if ($cart_data[0]["buyer_mobile"] == "")
                    $update_cart['buyer_mobile'] = $buyer_phone;
        ...
        if ($action == 'add') { // add item
            ...
            if (strpos($attribute_info_json, "'") !== false)
                $attribute_info_json = addslashes($attribute_info_json);
        ...
        if (!empty($attribute_info)) {
            ...
            if (isset($this_cart_item_data[0]))
                $this_cart_item = $this_cart_item_data[0];
        }
    ...
    public function update_cart($cart_id = 0, $subscriber_id_passed = '') {
        if ($subscriber_id_passed == '')
            $subscriber_id = $this->input->get_post("subscriber_id");
        else
            $subscriber_id = $subscriber_id_passed;
                ...
                if ($cart_data[0]["store_unique_id"] != "") { // store availabe and online
                    ...
                    foreach ($cart_item_data as $key => $value) {
                        ...
                        if (array_key_exists($product_id, $product_list_assoc)) {
                            ...
                            if ($product_list_assoc[$product_id]["taxable"] == '1')
                                $taxable_amount += $new_price * $value["quantity"];
                        }
                        else {
                    ...
                    if ($free_shipping_enabled == '0')
                        $shipping = mec_number_format($shipping_charge, $decimal_point, '0');
    ...
    public function apply_coupon() {
        ...
        if (isset($cart_data[0]) && $cart_data[0]["store_id"] != "") {
            ...
            if ($coupon_code == "") {
                if ($xcoupon_code != "") {
                    ...
                    echo json_encode(array('status' => '0', 'message' => $this->lang->line('No coupon code provided.')));
                exit();
            }
        }
        else {
        ...
        if (!empty($coupon_data)) {
            ...
            echo json_encode(array('status' => '0', 'message' => $this->lang->line('Invalid coupon code.')));
    }
    ...
    ' . $this->lang->line("Please login to continue.") . '<br><br><a href="" id="login_form" class="pointer btn btn-primary">' . $this->lang->line("Login to continue") . '</a>
        ...
        foreach ($country_names as $key => $value) {
            if ($country != '')
                $selected_country = ($key == $country) ? 'selected' : '';
            else
                $selected_country = ($key == $store_data[0]['store_country']) ? 'selected' : '';
        ...
        if ($state_var != '' || $city_var != '' || $zip_var != '')
            $state_city_street_html .= '<div class="form-group">
  <div class="input-group">
  ...
  ' . $state_var . $city_var . $zip_var . '
        ...
        if ($is_checkout_country == '1')
            $country_html .= '<div class="form-group">
  <div class="input-group">
  <div class="input-group-prepend">
  ...
  ' . $phone_html;
    ...
    public function save_profile_data() {
        ...
        if ($mobile !== '' && !$country_code_embeded)
 ...
 WHERE `subscribe_id` = '" . $subscriber_id . "' ";
  ...
  ' . $this->lang->line("Please login to continue.") . '<br><br><a href="" id="login_form" class="pointer btn btn-primary">' . $this->lang->line("Login to continue") . '</a>
        ...
        if (!isset($data_close) || $data_close == '')
            $data_close = '0';
  ...
  ' . $phone_html;
    ...
    public function proceed_checkout() {
        ...
        if ($subscriber_id != "")
            $where_subs = array("subscriber_type" => "system", "subscribe_id" => $subscriber_id, "store_id" => $tmp_store_id);
        else {
            if ($subscriber_id == "")
                $subscriber_id = isset($mydata['subscriber_id']) ? $mydata['subscriber_id'] : '';
            if ($subscriber_id != "")
        ...
        $paytm_enabled = $cart_data[0]["paytm_enabled"];
        if ($store_favicon != "")
            $store_favicon = base_url("upload/ecommerce/" . $store_favicon);
        ...
        $paytm_merchant_mid = isset($ecommerce_config['paytm_merchant_mid']) ? $ecommerce_config['paytm_merchant_mid'] : '';
        $paytm_merchant_key = isset($ecommerce_config['paytm_merchant_key']) ? $ecommerce_config['paytm_merchant_key'] : '';
        $paytm_mode = isset($ecommerce_config['paytm_mode']) ? $ecommerce_config['paytm_mode'] : 'live';
        if ($is_order_schedule == '1') {
            $check_delivery_time = $delivery_time;
            if ($check_delivery_time == "")
                $check_delivery_time = date("Y-m-d H:i:s");
            ...
            if (isset($ecommerce_store_business_hours[0])) {
                if ($ecommerce_store_business_hours[0]['off_day'] == '1') {
                    ...
                    if (!$time_ok) {
                        if ($delivery_time == '')
                            echo json_encode(array('status' => '2', 'message' => $this->lang->line('Sorry, but we cannot take the order. We are closed now.')));
                        else
                            echo json_encode(array('status' => '2', 'message' => $this->lang->line('Sorry, but we cannot take the order. We will be closed at the selected time. Please try selecting a new delivery time.')));
                        exit();
                    }
                }
            }
        ...
        if (!empty($delivery_address_id) && $store_pickup == '0') {
            ...
            $buyer_first_name = $buyer_last_name = $buyer_country = "";
        ...
        if ($paytm_enabled == '1' && ($paytm_merchant_key == '' || $paytm_merchant_mid == '')) {
            echo json_encode(array('status' => '0', 'message' => $this->lang->line('Paytm payment settings not found.')));
            exit();
        }
        ...
        $paypal_button = $stripe_button = $razorpay_button = $paystack_button = $mollie_button = $manual_button = $mercadopago_button = $sslcommerz_button = $senangpay_button = $instamojo_button = $xendit_button = $cod_button = $toyyibpay_button = $myfatoorah_button = $paytm_button = $paymaya_button = "";
        ...
        if ($paystack_enabled == "1") {
            ...
            if ($customer_email != "")
                $this->paystack_class_ecommerce->customer_email = $customer_email;
            else
                $this->paystack_class_ecommerce->customer_email = $fake_email;
  ...
  <h6 class="mb-1">' . $this->lang->line("Cash on Delivery") . '</h6>
        ...
        if ($paytm_enabled == "1") {
            //$this->lang->line("Pay with Paytm")
//  include_once(APPPATH."libraries/PaytmKit/lib/encdec_paytm.php");
//  $this->load->library('PaytmKit/lib/encdec_paytm.php');
//  $this->paytm_class_ecommerce->action_url= $this->paytm_response();
//  $this->paytm_class_ecommerce->mode=$paytm_mode;
//  $this->paytm_class_ecommerce->cancel_url=$cancel_url;
//  $this->paytm_class_ecommerce->success_url=$success_url;
//  $this->paytm_class_ecommerce->amount=$payment_amount;
//  $this->paytm_class_ecommerce->user_id=$user_id;
//  $this->paytm_class_ecommerce->currency=$currency;
//  $this->paytm_class_ecommerce->cart_id=$cart_id;
//  $this->paytm_class_ecommerce->customer_phone=$mydata['subscriber_first_name'];
//  $this->paytm_class_ecommerce->customer_email=$mydata['subscriber_first_name'];
//  $this->paytm_class_ecommerce->cus_id=$mydata['subscriber_id'];
//  $this->paytm_class_ecommerce->subscriber_id=$subscriber_id;
//  $this->paytm_class_ecommerce->button_lang=$this->lang->line("Pay with Paytm");
//  $this->paytm_class_ecommerce->secondary_button = true;
//  $paytm_button = $this->paytm_class_ecommerce->set_button();




            $query = $this->db->get('ecommerce_config');
            $config_data = $query->result_array();
            $paytm_merchant_mid = $config_data[0]['paytm_merchant_mid'];
            $paytm_mode = isset($config_data[0]['paytm_mode']) ? $config_data[0]['paytm_mode'] : "live";
            $paytm_merchant_key = $config_data[0]['paytm_merchant_key'];
            ...
            $url = site_url('ecommerce/response');
            $order_id = "ORDER-" . rand(10000, 99999999);
            $button = "";
            $paramList = array();
            $ORDER_ID = $order_id;
            $CUST_ID = $mydata['subscriber_id'];
            ...
            $paramList["CALLBACK_URL"] = site_url('ecommerce/response/' . $cart_id);
            $CALLBACK_URL = site_url('ecommerce/response/' . $cart_id);
            $paytmChecksum = getChecksumFromArray($paramList, $paytm_merchant_key);
            $transactionURL = $PAYTM_TXN_URL;
            $paytm_lang = !empty($this->lang->line("Pay with Paytm")) ? $this->lang->line("Pay with Paytm") : $this->lang->line("Pay with Paytm");
            $hide_me = TRUE ? 'display:none;' : '';
            $button = "";
            $button .= "<form action='{$transactionURL}' method='post' style='padding: 0; margin: 0;" . $hide_me . "' id='paytm_btn'>";
            $button .= "<input type='hidden' name='ORDER_ID' value='{$ORDER_ID}'/>";
            $button .= "<input type='hidden' name='CUST_ID' value='{$CUST_ID}' />";
            $button .= "<input type='hidden' name='TXN_AMOUNT' value='{$TXN_AMOUNT}' />";
            $button .= "<input type='hidden' name='MID' value='{$paytm_merchant_mid}' />";
            $button .= "<input type='hidden' name='CHANNEL_ID' value='{$CHANNEL_ID}' />";
            $button .= "<input type='hidden' name='WEBSITE' value='WEBSTAGING'>";
            $button .= "<input type='hidden' name='INDUSTRY_TYPE_ID' value='{$INDUSTRY_TYPE_ID}'>";
            $button .= "<input type='hidden' name='CHECKSUMHASH' value='{$paytmChecksum}' />";
            $button .= "<input type='hidden' name='CALLBACK_URL' value='{$CALLBACK_URL}' />";
            $button .= "<button type='submit' class='btn paypal_button_sn'>" . $this->lang->line("Pay with Paytm") . "</button>";
            $button .= "</form>";
            if (TRUE) {
                $button .= "
		<a href='#' class='list-group-item list-group-item-action flex-column align-items-start' id='paytm_clone' onclick=\"document.getElementById('paytm_btn').submit();\">
		    <div class='d-flex w-100 align-items-center'>
		      <small class='text-muted'><img class='rounded' width='60' height='60' src='" . base_url('assets/img/payment/Paytm-Logo.jpg') . "'></small>
		      <h6 class='mb-1'>" . $paytm_lang . "</h6>
		    </div>
		</a>";
            }
            ...
            $paytm_button = '<div class="col-12 col-md-6">' . $button . '</div>';
        }

        $html = '<div class="row">' . $cod_button . $paypal_button . $stripe_button . $razorpay_button . $paystack_button . $mollie_button . $mercadopago_button . $sslcommerz_button . $senangpay_button . $instamojo_button . $xendit_button . $manual_button . $toyyibpay_button . $myfatoorah_button . $paymaya_button . $paytm_button . '</div>';

        echo json_encode(array('status' => '1', 'message' => '', 'html' => $html));
    }

    public function response($cart_id = 0) {
...
//
                ...
                if (isset($_POST['STATUS']) && $_POST['STATUS'] == 'TXN_SUCCESS') {
                    $transaction_id = isset($_POST['TXNID']) ? $_POST['TXNID'] : 0;
                    $currency = isset($_POST['CURRENCY']) ? strtoupper($_POST['CURRENCY']) : 'INR';
                    $curtime = date("Y-m-d H:i:s");
                    $payment_date = isset($_POST['TXNDATE']) ? strtoupper($_POST['TXNDATE']) : $curtime;
                    $account_email = isset($cart_data[0]['buyer_email']) ? strtoupper($cart_data[0]['buyer_email']) : '';
                    $insert_data = array
                        (
                        'checkout_account_email' => 'test@gmail.com', //$account_email,
                    ...
                    if ($cart_id != "" && $subscriber_id != "") {
                        $cart_data = $this->valid_cart_data($cart_id, $subscriber_id, $select = "");
                        if (isset($cart_data[0]['store_locale']))
                            $this->_language_loader($cart_data[0]['store_locale']);
                    }
    ...
    public function manual_payment() {
        ...
        if (false === $this->form_validation->run()) {
            if ($this->form_validation->error('paid-amount'))
                $message = $this->form_validation->error('paid-amount');
            else if ($this->form_validation->error('paid-currency'))
                $message = $this->form_validation->error('paid-currency');
            else if ($this->form_validation->error('additional-info'))
                $message = $this->form_validation->error('additional-info');
            else if ($this->form_validation->error('cart_id'))
                $message = $this->form_validation->error('cart_id');
            else if ($this->form_validation->error('subscriber_id'))
                $message = $this->form_validation->error('subscriber_id');
            else
    ...
    public function cod_payment() {
        ...
        if ($this->basic->update_data('ecommerce_cart', array("id" => $cart_id, "subscriber_id" => $subscriber_id, "action_type !=" => "checkout"), $data)) {
            $get_data = $this->db->query("select * from ecommerce_cart where id='" . $cart_id . "'")->row();
            $notification_log = array('user_id' => $get_data->user_id,
                'store_id' => $get_data->store_id,
                'cart_id' => $cart_id,
            ...
            $this->db->insert('notification_log', $notification_log);
            $this->sendMobileNotification($cart_id, $amount = 0, $type = "new_order");
    ...
    public function stripe_action($store_id = '', $cart_id = '', $subscriber_id = '', $payment_amount = '', $currency = '', $store_name = '') {
        ...
        $notification_log = array('user_id' => $get_data->user_id,
            ...
            'created_date' => date("Y-m-d h:i:sa"));
        $this->db->insert('notification_log', $notification_log);
        $this->sendMobileNotification($cart_id, $payment_amount, $type = 'payment');
        redirect($invoice_link, 'location');
    }
    ...
    public function instamojo_action($cart_id = 0) {
        ...
        if ($customer_email != '')
            $this->instamojo->email = $customer_email;
        $this->instamojo->button_lang = $this->lang->line('Pay with Instamojo');
        ...
        if ($instamojo_mode == 'sandbox')
            $this->instamojo->instamojo_api_url = "https://test.instamojo.com/api/1.1/payment-requests/";
        else
            $this->instamojo->instamojo_api_url = "https://instamojo.com/api/1.1/payment-requests/";
    ...
    public function myfatoorah_success($cart_id = 0) {
        ...
        if ($myfatoorah_mode == 'sandbox')
            ...
            $this->myfatoorah->myfatoorah_api_url = "https://apitest.myfatoorah.com/v2/getPaymentStatus/";
    ...
    public function paypal_action_main() {
        ...
        if ($paypal_status_verification == '')
            $paypal_status_verification = '1';
    ...
    private function spin_and_replace($str = "", $replace = array(), $is_spin = true) {
        if (!isset($replace['store_name']))
            $replace['store_name'] = '';
        if (!isset($replace['store_url']))
            $replace['store_url'] = '';
        if (!isset($replace['order_no']))
            $replace['order_no'] = '';
        if (!isset($replace['order_url']))
            $replace['order_url'] = '';
        if (!isset($replace['checkout_url']))
            $replace['checkout_url'] = '';
        if (!isset($replace['my_orders_url']))
            $replace['my_orders_url'] = '';
        if (!isset($replace['first_name']))
            $replace['first_name'] = '';
        if (!isset($replace['last_name']))
            $replace['last_name'] = '';
        if (!isset($replace['email']))
            $replace['email'] = '';
        if (!isset($replace['mobile']))
            $replace['mobile'] = '';
    ...
    private function spin_and_replace_notification($str = "", $replace = array(), $is_spin = true) {
        ...
        if (!isset($replace['order_no']))
            $replace['order_no'] = '';
        if (!isset($replace['order_status']))
            $replace['order_status'] = '';
        if (!isset($replace['invoice_url']))
            $replace['invoice_url'] = '';
        if (!isset($replace['update_note']))
            $replace['update_note'] = '';
        if (!isset($replace['first_name']))
            $replace['first_name'] = '';
        if (!isset($replace['last_name']))
    ...
    private function confirmation_message_sender($cart_id = 0, $subscriber_id = "") {
        ...
        if (empty($buyer_email))
            $buyer_email = isset($cart_data['bill_email']) ? $cart_data['bill_email'] : '';
        if (empty($buyer_mobile))
            $buyer_mobile = isset($cart_data['bill_mobile']) ? $cart_data['bill_mobile'] : '';
        ...
        if ($action_type == 'checkout') {
            ...
            foreach ($cart_info as $key => $value) {
                ...
                if ($value['thumbnail'] == '')
                    $image_url = base_url('assets/img/products/product-1.jpg');
                else
                    $image_url = base_url('upload/ecommerce/' . $value['thumbnail']);
    ...
    public function my_orders($store_id = 0) {
        ...
        $store_data = $this->basic->get_data("ecommerce_store", array("where" => array("id" => $store_id)), "store_name,slug,store_unique_id,slug,store_logo,store_favicon,terms_use_link,refund_policy_link,store_locale,pixel_id,google_id,id,user_id");
        if ($store_id == 0 || !isset($store_data[0])) {
            $not_found = $this->lang->line("Order data not found.");
            echo '<br/><h1 style="text-align:center">' . $not_found . '</h1>';
            exit();
    ...
    public function my_orders_data() {
        ...
        if ($search_status != "")
            $this->db->where(array("ecommerce_cart.status" => $search_status));
        ...
        foreach ($info as $key => $value) {
            ...
            if ($info[$key]['payment_method'] == 'Cash on Delivery')
                $pay = "Cash";
            else
                $pay = $info[$key]['payment_method'];
            ...
            if ($value["action_type"] == "checkout"){
                $invoice = base_url("ecommerce/order/" . $value['id']);
                $download_invoice =  base_url("ecommerce/download_invoice/" . $value['id']);
            }else{
                $invoice = base_url("ecommerce/cart/" . $value['id']);
            $invoice = mec_add_get_param($invoice, array("subscriber_id" => $subscriber_id, "pickup" => $pickup));
            }
            $payment_status = $info[$key]['status'];

            if ($payment_status == 'pending')
                $payment_status_badge = "<span class='text-danger'>" . $this->lang->line("Pending") . "</span>";
            else if ($payment_status == 'approved')
                $payment_status_badge = "<span class='text-primary'>" . $this->lang->line("Approved") . "</span>";
            else if ($payment_status == 'rejected')
                $payment_status_badge = "<span class='text-danger'>" . $this->lang->line("Rejected") . "</span>";
            else if ($payment_status == 'shipped')
                $payment_status_badge = "<span class='text-info'>" . $this->lang->line("Shipped") . "</span>";
            else if ($payment_status == 'delivered')
                $payment_status_badge = "<span class='text-info'>" . $this->lang->line("Delivered") . "</span>";
            else if ($payment_status == 'completed')
                $payment_status_badge = "<span class='text-success'>" . $this->lang->line("Completed") . "</span>";
    ...
    <a href="' . $download_invoice . '" class="dropdown-item has-icon"><i class="fas fa-file-pdf"></i> ' . $this->lang->line("Download") . '</a>
    ' . $st1 . '
    ' . $st2 . '
    </div>
    </div>
    ...
    public function order_list_data() {
        ...
        foreach ($info as $key => $value) {
            ...
            if (trim($info[$key]['payment_method']) == "")
                $info[$key]['payment_method'] = "x";
            ...
            $disabled_status = '';
            if ($info[$key]['store_type'] == 'digital')
...
//    public function get_order_dashboard_data() {
//        $this->ajax_check();
//        $ecommerce_config = $this->get_ecommerce_config();
//        $currency_position = isset($ecommerce_config['currency_position']) ? $ecommerce_config['currency_position'] : "left";
//        $decimal_point = isset($ecommerce_config['decimal_point']) ? $ecommerce_config['decimal_point'] : 0;
...
//    }
    ...
    public function order_dashboard($search_param = '') {
        $store_list = $this->get_store_list();
        ...
        $data['search_param'] = $search_param;
        $data['status_list'] = $this->get_payment_status();
        $data['body'] = 'ecommerce/order_dashboard';
        $data['page_title'] = $this->lang->line('Orders') . " : " . $this->session->userdata("ecommerce_selected_store_title");
        //$data["iframe"] = "1";
        $store_id = $this->session->userdata("ecommerce_selected_store");
        $data['order'] = $this->db->query("SELECT ecommerce_cart.id,ecommerce_cart.user_id,store_id,subscriber_id,coupon_code,coupon_type,discount,payment_amount,currency,ordered_at,transaction_id,card_ending,payment_method,manual_additional_info,manual_filename,paid_at,ecommerce_cart.status,ecommerce_cart.updated_at,ecommerce_cart.action_type,ecommerce_store.store_name,ecommerce_store.store_type FROM ecommerce_cart INNER JOIN ecommerce_store ON ecommerce_store.id=ecommerce_cart.store_id WHERE ecommerce_cart.user_id ='" . $this->user_id . "' AND ecommerce_cart.store_id ='" . $store_id . "' AND ecommerce_cart.status='pending' AND ecommerce_cart.action_type = 'checkout'")->result();

        $this->load->view('ecommerce/order_dashboard', $data);

//        $this->_viewcontroller($data);
    }

    public function is_order_pending() {
        $store_id = $this->session->userdata("ecommerce_selected_store");
        $order = $this->db->query("SELECT ecommerce_cart.id,ecommerce_cart.user_id,store_id,subscriber_id,coupon_code,coupon_type,discount,payment_amount,currency,ordered_at,transaction_id,card_ending,payment_method,manual_additional_info,manual_filename,paid_at,ecommerce_cart.status,ecommerce_cart.updated_at,ecommerce_cart.action_type,ecommerce_store.store_name,ecommerce_store.store_type FROM ecommerce_cart INNER JOIN ecommerce_store ON ecommerce_store.id=ecommerce_cart.store_id WHERE ecommerce_cart.user_id ='" . $this->user_id . "' AND ecommerce_cart.store_id ='" . $store_id . "' AND ecommerce_cart.status='pending' AND ecommerce_cart.action_type = 'checkout' ORDER BY id DESC")->result();
        $status = count($order);
        if ($status > 0) {
            echo '1';
        } else {
...
//        $shipped_through = $this->input->post("shipped_through", true);
//        $store_id = $this->input->post("store_id", true);
...
//        $store_email = $query[0]->store_email;
//        $store_phone = $query[0]->store_phone;
//        $store_country = $query[0]->store_country;
//        $store_city = $query[0]->store_city;
//        $store_state = $query[0]->store_state;
//        $store_zip = $query[0]->store_zip;
//        $store_address = $query[0]->store_address;
//        if ($shipped_through == 'shiprocket') {
//            $shiprocket_api_key = @$query[0]->shiprocket_api_key;
//            $shiprocket_user_name = @$query[0]->shiprocket_user_name;
//            $order_ids = $cart_data[0]['id'];
//
//            $post = '{
//  "order_id": "11313",
//  "order_date": "2021-12-06 04:89",
//  "pickup_location": "New Delhi",
//  "channel_id": "",
//  "comment": "Reseller: M/s Goku",
//  "billing_customer_name": "A",
//  "billing_last_name": "B",
...
//  "weight": 2.5
//}';
//            $this->shiprocket->generateOrder($post);
//            $this->shiprocket->generatePickup($store_zip);
//        }
//        if ($shipped_through == 'dunzo') {
//            $dunzo_secret_key = @$query[0]->dunzo_secret_key;
//            $dunzo_client_id = @$query[0]->dunzo_client_id;
//        }
//        if ($shipped_through == 'borzo') {
//            $borzo_authentication_Key = @$query[0]->borzo_authentication_Key;
//            $borzo_secret_key = @$query[0]->borzo_secret_key;
//        }
//        if ($shipped_through == 'rapido') {
//            $rapido_user_name = @$query[0]->rapido_user_name;
//            $rapido_secret_key = @$query[0]->rapido_secret_key;
//        }
        //$join = array('ecommerce_store'=>"ecommerce_store.id=ecommerce_cart.store_id,left");;
        //$cart_data = $this->basic->get_data('ecommerce_cart',array("where"=>array("ecommerce_cart.id"=>$id)),"buyer_email,buyer_mobile,buyer_first_name,buyer_last_name,bill_email,bill_mobile,ecommerce_cart.id,subscriber_id,page_id,store_name,store_unique_id,ecommerce_store.id as store_id,notification_message,notification_sms_api_id,notification_email_api_id,notification_email_subject,notification_configure_email_table",$join);
        if (!isset($cart_data[0])) {
    ...
    public function reminder_send_status_data() {
        ...
        foreach ($info as $key => $value) {
            if ($info[$key]['is_sent'] == '1' && $info[$key]['sent_at'] != "0000-00-00 00:00:00")
                ...
                $sent_time = '<span class="text-muted"><i class="fas fa-exclamation-circle"></i> ' . $this->lang->line("Not Sent") . "<span>";
    ...
    public function add_store_action() {
        ...
        if ($refund_policy_link == "<p></p>")
            $refund_policy_link = "";
        if ($terms_use_link == "<p></p>")
            $terms_use_link = "";
    ...
    public function reminder_settings_action() {
        ...
        if ($msg_checkout_text == "")
            $msg_reminder_text_checkout_next = $reminder_default['messenger']['checkout']['checkout_text'];
        if ($msg_reminder_text_checkout_next == "")
            $msg_reminder_text_checkout_next = $reminder_default['messenger']['checkout']['checkout_text_next'];
        ...
        if ($this->session->userdata('user_type') == 'Admin' || in_array(263, $this->module_access)) {
            ...
            if (isset($email_api_id) && $email_api_id != "") {
                if ($email_subject == "")
                    $email_subject = "{{store_name}} | Order Update";
    ...
    public function qr_code($id = 0, $pickup_point_id = '') {
        ...
        if ($pickup_point_id != "")
            $reference = $reference . "-" . $pickup_point_id;
        ...
        if ($page_id != '') {
            ...
            if ($pickup_point_id == "")
                ...
                $filename = "ecommerce1-" . $this->user_id . '-' . $xdata[0]["store_unique_id"] . '-' . $pickup_point_id . ".png";
            $qrc->create($str, $filename, $qrc::QRC_ECLEVEL_L, 8, 1, false, $msg_back_color, $msg_fore_color);
        }

        $str2 = base_url('store/' . $xdata[0]['slug']);
        if ($pickup_point_id != "")
            $str2 = $str2 . "?pickup=" . $pickup_point_id;
        $qrc2 = $this->quick_response_code;
        if ($pickup_point_id == "")
            $filename2 = "ecommerce2-" . $this->user_id . '-' . $xdata[0]["store_unique_id"] . ".png";
        else
            $filename2 = "ecommerce2-" . $this->user_id . '-' . $xdata[0]["store_unique_id"] . '-' . $pickup_point_id . ".png";
    ...
    public function qr_code_action() {
        ...
        if ($msg_text == "")
            $msg_text = "Hi {{first_name}}, welcome to our store.";
        if ($msg_btn == "")
            $msg_btn = "START";
    ...
    public function edit_store_action() {
        ...
        $store = $xdata[0]['store_name'];

        $check_store = $this->db->query("select * from ecommerce_store where store_name='" . $store_name . "'")->result();
        $temp_store = @$check_store[0]->store_name;
        if ($temp_store == $store_name) {
            $store_names = $store;
        } else {
        ...
        $insert_data = array(
            ...
            "slug" => url_title(strtolower($store_names)),
            "store_email" => $store_email,
            "store_phone" => $store_phone,
            "store_country" => $store_country,
            "store_state" => $store_state,
        ...
        if ($store_logo != '') {
            ...
            if ($xstore_logo != '')
                @unlink('upload/ecommerce/' . $xstore_logo);
        }
        if ($store_favicon != '') {
            $insert_data["store_favicon"] = $store_favicon;
            if ($xstore_favicon != '')
                @unlink('upload/ecommerce/' . $xstore_favicon);
        }
    ...
    public function product_list_data() {
        ...
        if ($store_id != "")
            $this->db->where(array("ecommerce_product.store_id" => $store_id));
        ...
        if ($currency_position == 'left')
            $currency_left = $config_currency_icon;
        if ($currency_position == 'right')
            $currency_right = $config_currency_icon;
    ...
    $(\'[data-toggle="tooltip"]\').tooltip();
            ...
            if ($info[$key]['taxable'] == 1)
                $info[$key]['taxable'] = "<span class='badge badge-status text-success'><i class='fa fa-check-circle green'></i> " . $this->lang->line('Yes') . "</span>";
            else
                $info[$key]['taxable'] = "<span class='badge badge-status text-danger'><i class='fa fa-times red'></i> " . $this->lang->line('No') . "</span>";

            if ($info[$key]['sell_price'] > 0)
                $info[$key]['original_price'] = "<span style='text-decoration: line-through;' class='text-muted'>" . $currency_left . mec_number_format($info[$key]['original_price'], $decimal_point, $thousand_comma) . $currency_right . "</span> <b class='text-warning'>" . $currency_left . mec_number_format($info[$key]['sell_price'], $decimal_point, $thousand_comma) . $currency_right . "</b>";
            else
                $info[$key]['original_price'] = "<b>" . $currency_left . mec_number_format($info[$key]['original_price'], $decimal_point, $thousand_comma) . $currency_right . "</b>";

            if ($info[$key]['thumbnail'] == '')
                $url = base_url('assets/img/products/product-1.jpg');
            else
                $url = base_url('upload/ecommerce/' . $info[$key]['thumbnail']);
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if (!isset($related_product_ids) || !is_array($related_product_ids) || empty($related_product_ids))
                    ...
                    $related_product_ids = implode(',', $related_product_ids);
                ...
                if (!isset($attribute_ids) || !is_array($attribute_ids) || empty($attribute_ids))
                    $attribute_ids = '';
                else
    ...
    public function edit_product_action() {
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                // $store_id=$this->input->post('store_id',true);
                $category_id = $this->input->post('category_id', true);
                $sub_category_id=$this->input->post('sub_category', true);
                $attribute_ids = $this->input->post('attribute_ids', true);
                $product_name = strip_tags($this->input->post('product_name', true));
                $original_price = $this->input->post('original_price', true);
                $sell_price = $this->input->post('sell_price', true);
                ...
                if ($thumbnail != '') {
                    ...
                    if ($xthumbnail != '')
                        @unlink('upload/ecommerce/' . $xthumbnail);
                }

                if ($product_file != '') {
                    $data['digital_product_file'] = $product_file;
                    if ($xdigital_product_file != '')
                        @unlink('upload/ecommerce/digital_product/' . $xdigital_product_file);
                }
                if ($featured_images != '') {
    ...
    public function payment_accounts() {
        ...
        if ($this->is_demo == '1')
            $data["xvalue"]["stripe_secret_key"] = $data["xvalue"]["stripe_publishable_key"] = $data["xvalue"]["paypal_email"] = $data["xvalue"]["paystack_secret_key"] = $data["xvalue"]["paystack_public_key"] = $data["xvalue"]["razorpay_key_id"] = $data["xvalue"]["razorpay_key_secret"] = $data["xvalue"]["mollie_api_key"] = $data["xvalue"]["mercadopago_public_key"] = $data["xvalue"]["mercadopago_access_token"] = $data["xvalue"]["sslcommerz_store_id"] = $data["xvalue"]["sslcommerz_store_password"] = $data["xvalue"]["senangpay_merchent_id"] = $data["xvalue"]["senangpay_secret_key"] = $data["xvalue"]["instamojo_api_key"] = $data["xvalue"]["instamojo_auth_token"] = $data["xvalue"]["xendit_secret_api_key"] = $data["xvalue"]["toyyibpay_secret_key"] = $data["xvalue"]["toyyibpay_category_code"] = "XXXXXXXXXX";
   ...
   public function payment_accounts_action()
{
  ...
  if ($_POST)
  {
    ...
    $this->form_validation->set_rules('dunzo_secret_key','<b>'.$this->lang->line("Dunzo Secret Key").'</b>','trim');
    $this->form_validation->set_rules('dunzo_client_id','<b>'.$this->lang->line("Dunzo Client ID").'</b>','trim');
    $this->form_validation->set_rules('dunzo_enabled','<b>'.$this->lang->line("Dunzo Enabled").'</b>','trim');
    $this->form_validation->set_rules('borzo_secret_key','<b>'.$this->lang->line("Borzo Secret Key").'</b>','trim');
    $this->form_validation->set_rules('borzo_authentication_Key','<b>'.$this->lang->line("Borzo Authentication Key").'</b>','trim');
    $this->form_validation->set_rules('borzo_enabled','<b>'.$this->lang->line("Borzo Enabled").'</b>','trim');

    $this->form_validation->set_rules('rapido_secret_key','<b>'.$this->lang->line("Rapido Secret Key").'</b>','trim');
    $this->form_validation->set_rules('rapido_user_name','<b>'.$this->lang->line("Rapido User Name").'</b>','trim');
    $this->form_validation->set_rules('rapido_enabled','<b>'.$this->lang->line("Rapido Enabled").'</b>','trim');
    $this->form_validation->set_rules('shiprocket_api_key','<b>'.$this->lang->line("Shiprocket API Key").'</b>','trim');
    $this->form_validation->set_rules('shiprocket_user_name','<b>'.$this->lang->line("Shiprocket User Name").'</b>','trim');
    $this->form_validation->set_rules('shiprocket_enabled','<b>'.$this->lang->line("Shiprocket Enabled").'</b>','trim');
    ...
    else
    {
      ...
      $gst_no=strip_tags($this->input->post('gst_no',true));
      ...
      $dunzo_secret_key=$this->input->post('dunzo_secret_key',true);
      $dunzo_client_id=$this->input->post('dunzo_client_id',true);
      $dunzo_enabled=$this->input->post('dunzo_enabled',true);
      $borzo_secret_key=$this->input->post('borzo_secret_key',true);
      $borzo_authentication_Key=$this->input->post('borzo_authentication_Key',true);
      $borzo_enabled=$this->input->post('borzo_enabled',true);
      $rapido_secret_key=$this->input->post('rapido_secret_key',true);
      $rapido_user_name=$this->input->post('rapido_user_name',true);
      $shiprocket_api_key=$this->input->post('shiprocket_api_key',true);
      $shiprocket_user_name=$this->input->post('shiprocket_user_name',true);
      $rapido_enabled=$this->input->post('rapido_enabled',true);
      $shiprocket_enabled=$this->input->post('shiprocket_enabled',true);
      $store_type=$this->input->post('store_type',true);
      if($store_type == 'digital') {
  ...
  {{delivery_address}}';
            ...
            if ($this->form_validation->run() == false) {
                ...
                if (!empty($_FILES['slider_image']['name'])) {
                    ...
                    foreach ($uploadData as $val) {
                        $temp_array .= $val['file_name'] . ',';
                    }
                    $slider_id = $this->input->post('slider_id');
                    $slider_data = array('user_id' => $this->session->userdata('user_id'),
                        'store_id' => $this->session->userdata('ecommerce_selected_store'),
                        ...
                        'created_date' => time());
                    if ($slider_id != '') {
                        $q = $this->db->query("select * from slider_image where id=$slider_id")->row();
                        $temp_array .= $q->slider_img;
                        $this->db->where('id', $slider_id);
                        $this->db->set('slider_img', $temp_array);
    ...
    public function delete_slider_image() {
        ...
        $slider_img = $this->input->post('img');
        $query = $this->db->query("select * from slider_image where id=$slider_id")->row();
        $mystring = $query->slider_img;
        $result_string = $this->removeItemString($mystring, $slider_img);

        $this->db->where('id', $slider_id);
        $this->db->set('slider_img', $result_string);
        $this->db->update('slider_image');
...
//    sub category
    ...
    public function sub_category_list() {
        $data['body'] = 'ecommerce/sub_category_list';
        $data['page_title'] = $this->lang->line('Category') . " : " . $this->session->userdata("ecommerce_selected_store_title");
        $store_list = $this->get_store_list();
        $store_list[''] = $this->lang->line("Store");
        $data['store_list'] = $store_list;
...
//        $data['category'] = $this->db->query("SELECT id,name FROM `category` WHERE is_deleted=0")->result();
    ...
    public function sub_category_list_data() {
        $search_value = $_POST['search']['value'];
        $display_columns = array("#", 'CHECKBOX', 'serial', 'thumbnail', 'sub_category_name', 'category_name', 'store_name', 'status', 'actions', 'updated_at',);
        $search_columns = array('sub_category_name', 'store_name');
        ...
        $where_custom = '';
        $where_custom = "ecommerce_sub_category.user_id = " . $this->user_id . " AND ecommerce_sub_category.store_id = " . $this->session->userdata("ecommerce_selected_store");
        if ($search_value != '') {
            foreach ($search_columns as $key => $value)
                $temp[] = $value . " LIKE " . "'%$search_value%'";
            $imp = implode(" OR ", $temp);
        ...
        $table = "ecommerce_sub_category";
        $select = "ecommerce_sub_category.*,ecommerce_store.store_name";
        $join = array('ecommerce_store' => "ecommerce_store.id=ecommerce_sub_category.store_id,left");
        $this->db->where($where_custom);
        $info = $this->basic->get_data($table, $where = '', $select, $join, $limit, $start, $order_by, $group_by = '');

        $this->db->where($where_custom);
        $total_rows_array = $this->basic->count_row($table, $where = '', $count = "ecommerce_sub_category.id", $join, $group_by = '');
        $total_result = $total_rows_array[0]['total_rows'];

        foreach ($info as $key => $value) {
            $cat_id = $info[$key]['category_id'];
            $info[$key]['actions'] = "<div style='min-width:100px'><a href='#' title='" . $this->lang->line("Edit") . "' data-toggle='tooltip' class='btn btn-circle btn-outline-warning edit_row' table_id='" . $info[$key]['id'] . "'><i class='fa fa-edit'></i></a>&nbsp;&nbsp;";
            $category_name = @$this->db->query("select category_name from ecommerce_category where id='" . $cat_id . "'")->row()->category_name;
            $info[$key]['category_name']=$category_name;
            $info[$key]['actions'] .= "<a href='#' title='" . $this->lang->line("Delete") . "' data-toggle='tooltip' class='btn btn-circle btn-outline-danger delete_row' table_id='" . $info[$key]['id'] . "'><i class='fa fa-trash-alt'></i></a></div>
    <script>$('[data-toggle=\"tooltip\"]').tooltip();</script>";
    ...
    public function ajax_create_new_sub_category() {
        $this->ajax_check();
        $result = array('status' => '0', 'message' => $this->lang->line("Something went wrong, please try once again."));

        if ($_POST) {
            $category_id = strip_tags($this->input->post("category_id", true));
            $sub_category_name = strip_tags($this->input->post("sub_category_name", true));
            $store_id = $this->input->post("store_id", true);
            $store_id = $this->input->post("store_id", true);
            $thumbnail = $this->input->post('thumbnail', true);
            ...
            if ($this->basic->insert_data("ecommerce_sub_category", $inserted_data)) {
                $result['status'] = "1";
                $result['message'] = $this->lang->line("Sub Category has been added successfully.");
            }
    ...
    public function ajax_get_sub_category_update_info() {
        ...
        $details = $this->basic->get_data("ecommerce_sub_category", array('where' => array('id' => $table_id, 'user_id' => $user_id)));
        $selected = ($details[0]['status'] == '1') ? 'checked' : '';

        $store_list = $this->get_store_list();
        $store_list[''] = $this->lang->line("Store");
        $all_categories = $this->basic->get_data("ecommerce_category", $where = array('where' => array("store_id" => $this->session->userdata("ecommerce_selected_store"))));
        $form = '<div class="row">
  <div class="col-12">
  <form action="#" enctype="multipart/form-data" id="row_update_form" method="post">
  <input type="hidden" name="table_id" value="' . $table_id . '">
  ...
  <div class="col-12">
                                    <div class="form-group">
                                        <label for="name">' . $this->lang->line("Category") . '*</label>
                                        <select name="category_id" id="category_id" class="form-control select2" style="width:100%">
                                            <option value="">' . $this->lang->line("Category") . '</option>';

        foreach ($all_categories as $cat) {
            if ($cat['id'] == $details[0]['category_id']) {

                $form .= '<option selected value="' . $cat['id'] . '">' . $cat['category_name'] . '</option>';
            } else {
                $form .= '<option value="' . $cat['id'] . '">' . $cat['category_name'] . '</option>';
            }
        }
  ...
  <div class="col-12">
  <div class="form-group">
  <label>' . $this->lang->line('Sub Category Name') . ' *</label>
  <input type="text" class="form-control" name="sub_category_name2" id="sub_category_name2" value="' . $details[0]['sub_category_name'] . '">
  </div>
  </div>
  <div class="col-12">
  <div class="form-group">
  ...
  $(document).ready(function() {
    ...
    public function ajax_update_sub_category() {
        $this->ajax_check();
        $result = array('status' => '0', 'message' => $this->lang->line("Something went wrong, please try once again."));
        ...
        if ($_POST) {
            $table_id = $this->input->post("table_id", true);
            $category_id = strip_tags($this->input->post("category_id", true));
            $sub_category_name2 = strip_tags($this->input->post("sub_category_name2", true));
            $thumbnail = $this->input->post("thumbnail2", true);
            ...
            $updated_data = array
                (
                "category_id" => $category_id,
                "sub_category_name" => $sub_category_name2,
                "status" => $status,
                "updated_at" => date("Y-m-d H:i:s")
            );
            if ($thumbnail != '') {
            ...
            if ($this->basic->update_data("ecommerce_sub_category", array("id" => $table_id, "user_id" => $this->user_id), $updated_data)) {
                $result['status'] = "1";
                $result['message'] = $this->lang->line("Sub Category has been updated successfully.");
            }
    ...
    public function delete_sub_category() {
        ...
        if ($this->basic->delete_data("ecommerce_sub_category", array("id" => $table_id, 'user_id' => $this->user_id)))
            echo json_encode(array('message' => $this->lang->line("Sub Category has been deleted successfully."), 'status' => '1'));
        else
            echo json_encode($result);
    }
    ...
    public function coupon_list_data() {
        ...
        foreach ($info as $key => $value) {
            ...
            if ($info[$key]['free_shipping_enabled'] == 1)
                $info[$key]['free_shipping_enabled'] = "<span class='badge badge-status text-success'><i class='fa fa-check-circle green'></i> " . $this->lang->line('Enabled') . "</span>";
            else
                $info[$key]['free_shipping_enabled'] = "<span class='badge badge-status text-danger'><i class='fa fa-times red'></i> " . $this->lang->line('Disabled') . "</span>";

            if ($info[$key]['max_usage_limit'] == '' || $info[$key]['max_usage_limit'] == '0')
                $info[$key]['max_usage_limit'] = 'âˆž';
    ...
    public function add_coupon_action() {
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if ($this->basic->insert_data('ecommerce_coupon', $data))
                    $this->session->set_flashdata('success_message', 1);
                else
                    $this->session->set_flashdata('error_message', 1);
    ...
    public function edit_coupon_action() {
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if ($this->basic->update_data('ecommerce_coupon', array("id" => $id, "user_id" => $this->user_id), $data))
                    $this->session->set_flashdata('success_message', 1);
                else
                    $this->session->set_flashdata('error_message', 1);
                redirect('ecommerce/coupon_list', 'location');
    ...
    public function delete_featured_image() {
        ...
        if (!isset($filename_exp['1']) || $filename_exp['1'] != $this->user_id)
            exit();
    ...
    private function valid_cart_data($cart_id = 0, $subscriber_id = "", $select = "") {
        ...
        if ($select == "")
            $select = array("ecommerce_cart.*", "tax_percentage", "shipping_charge", "store_unique_id", "store_locale");
    ...
    public function get_coupon_data($coupon_code = '', $store_id = '0') {
        ...
        if (isset($data[0])) {
            if ($data[0]['max_usage_limit'] > 0 && $data[0]['used'] == $data[0]["max_usage_limit"])
                return array();
            else
                return $data[0];
        } else
    ...
    private function get_current_store_data() {
        $data = $this->basic->get_data("ecommerce_store", array("where" => array("id" => $this->session->userdata("ecommerce_selected_store"), "status" => "1")), "store_type,store_name,id,store_unique_id,slug");
        if (isset($data[0]))
            return $data[0];
        else
            return array();
    ...
    private function get_current_cart($subscriber_id = "", $store_id = 0, $pickup = "") {
        ...
        if ($store_id != 0 && $subscriber_id != "") {
            ...
            if ($subscriber_id != "")
                ...
                $where_simple["ecommerce_cart.user_id"] = $this->user_id;
            $where = array('where' => $where_simple);
            ...
            foreach ($cart_data as $key => $value) {
                if ($value["quantity"] <= 0) {
                    ...
                    $cart_data_final[$value['product_id']] = $value;
            }
    ...
    public function get_product_list($store_id = '0', $ajax = '0', $multiselect = '0') {
        ...
        if ($ajax == '0')
            ...
            if ($multiselect == '0')
                echo form_dropdown('product_id', $product_info, '', 'class="form-control select2" id="product_id"');
            else
                echo form_dropdown('product_ids[]', $product_info, '0', 'class="form-control select2" id="product_ids" multiple');
            echo "<script>$('.select2').select2();</script>";
        }
    }
    ...
    private function get_payment_status() {
        return array('pending' => $this->lang->line('Pending'), 'approved' => $this->lang->line('Approved'), 'rejected' => $this->lang->line('Rejected'), 'delivered' => $this->lang->line('Delivered'), 'completed' => $this->lang->line('Completed'));
    }
    ...
    private function get_sms_api() {
        ...
        foreach ($sms_api_config as $info) {
            ...
            if ($info['phone_number'] != "")
                ...
                $sms_api_config_option[$id] = $info['gateway_name'] . ": " . $info['custom_name'];
        }
            ...
            switch (strtolower($info['extension'])) {
                ...
                case 'txt':
                    if (!$format)
                        ...
                        return '<a data-id="' . $id . '" id="mp-download-file" href="#" class="dropdown-item has-icon"><i class="fas fa-download"></i> ' . $this->lang->line("Download Attachment") . '</a>';
            }
        }
    }
    ...
    private function manual_payment_display_attachment($file, $format = false) {
        ...
        if ($format)
            $output .= '<a data-image="' . $file . '" href="' . $file . '" class="dropdown-item has-icon mp-img-item"><i class="fas fa-image"></i> ' . $this->lang->line("View Attachment") . '</a>';
        else {
    ...
    public function download_csv() { // order or contact
        ...
        $join = '';
        if (!empty($latest_order_list_sql))
            $this->db->where($latest_order_list_sql);
    ...
    public function get_price_basedon_attribues() {
        ...
        $response = [];
        if (!isset($attribute_info) || !is_array($attribute_info))
    ...
    public function shiprocket_create_order() {
//print_r($_POST);
        //$this->generateOrder();
    }
    ...
    public function check_store_name() {
        ...
        $query = $this->db->query('select count(store_name) as total_store from ecommerce_store where store_name="' . $store_name . '"')->result();
        if ($query[0]->total_store == 1) {
            echo "Already taken";
        } else {
            echo "Available";
        }
    ...
    public function product_bulk_upload() {
        $data['body'] = 'ecommerce/product_bulk_upload';
        $data['page_title'] = $this->lang->line('Product Upload') . " : " . $this->session->userdata("ecommerce_selected_store_title");
        $store_list = $this->get_store_list();
        $store_list[''] = $this->lang->line("Select Store");
        $data['store_list'] = $store_list;
        $get_store_type = $this->basic->get_data("ecommerce_store", array("where" => array('id' => $this->session->userdata("ecommerce_selected_store"))), array("store_type"));
        ...
        $data['product_lists'] = $product_lists;
        $store_id = $this->session->userdata("ecommerce_selected_store");
        $category_list = $this->db->query("select * from ecommerce_category where store_id=$store_id and status='1'")->result_array();
        $data['category_list'] = $category_list;
        $attribute_list = $this->get_attribute_list();
        $data['attribute_list'] = $attribute_list;
        // $this->is_ecommerce_related_product_addon_exist = false;
    ...
    public function product_bulk_upload_save() {
        //if(demo()){
        //$this->session->set_flashdata('error',$this->lang->line('This operation is invalid for demo'));
        //redirect('ecommerce/product_bulk_upload');
        // }

        if (!file_exists($_FILES['bulk_file']['tmp_name']) || !is_uploaded_file($_FILES['bulk_file']['tmp_name'])) {
            $this->session->set_flashdata('error', $this->lang->line('File is not selected'));
            redirect('ecommerce/product_bulk_upload');
        }
        ...
        if (!empty($products)) {
            $store_id = $this->session->userdata("ecommerce_selected_store");
            $total_pro = $this->db->query("SELECT COUNT(*) AS total_product FROM `ecommerce_product` WHERE store_id=$store_id")->result();
            if ($total_pro[0]->total_product < 500) {
                foreach ($products as $product) {
                    $this->product_bulk_upload_save_single($product);
                }
            } else {
                $this->session->set_flashdata('success', $this->lang->line('you are already added 100 product.!'));
                redirect('ecommerce/product_bulk_upload');
            }
        }

        $this->session->set_flashdata('error', $this->lang->line('Products uploaded'));
        redirect('ecommerce/product_bulk_upload');
    }
    ...
    public function product_bulk_upload_save_single($product) {
        ...
        if (!empty($product['thumbnail'])) {
            $image_urls = explode(',', $product['thumbnail']);
            //$product_data['num_of_imgs'] = count($image_urls);
        }
        if (!empty($product['featured_images'])) {
            $image_urls_1 = explode(',', $product['featured_images']);
            //$product_data['num_of_imgs_1'] = count($image_urls_1);
        }
        ...
        $product_data['original_price'] = is_numeric($product['original_price']) ? $product['original_price'] : 0;
        $product_data['sell_price'] = is_numeric($product['sell_price']) ? $product['sell_price'] : 0;
        $product_data['taxable'] = is_numeric($product['taxable']) ? $product['taxable'] : 0;
        $product_data['stock_item'] = is_numeric($product['stock_item']) ? $product['stock_item'] : 0;
        $product_data['stock_display'] = is_numeric($product['stock_display']) ? $product['stock_display'] : 0;
        $product_data['purchase_note'] = $product['purchase_note'];
        $product_data['thumbnail'] = $product['thumbnail'];
        $product_data['featured_images'] = $product['featured_images'];
        $product_data['category_id'] = $product['category_id'];
        $product_data['status'] = $product['status'];
        $product_data['is_featured'] = $product['is_featured'];
        $product_data['updated_at'] = date('Y-m-d H:i:s');

        $this->db->insert('ecommerce_product', $product_data);
        $product_id = $this->db->insert_id();
        ...
        if (!empty($image_urls)) {
            ...
            foreach ($image_urls as $url) {
                ...
                $file_name = pathinfo($url, PATHINFO_FILENAME) . $ext;
                file_put_contents('upload/ecommerce/' . $file_name, file_get_contents($url));
                // $ib = $this->file_exist_ret2('product', $product_id, $ib,$ext);
                // $this->img_thumb('product', $product_id . '_' . $ib, $ext);
                $data = [
                    'thumbnail' => $file_name,
                ];
                $this->db->where('id', $product_id);
                $this->db->update('ecommerce_product', $data);
            }
        }
        if (!empty($image_urls_1)) {

            $file_name_2 = '';
            foreach ($image_urls_1 as $url_1) {
                $result = '';
                $file_name_1 = '';
                $ext = '.' . pathinfo($url_1, PATHINFO_EXTENSION);
                $file_name_1 .= pathinfo($url_1, PATHINFO_FILENAME) . $ext . ',';
                $file_name_2 .= pathinfo($url_1, PATHINFO_FILENAME) . $ext . ',';
                $result = str_replace(',', '', $file_name_1);
                file_put_contents('upload/ecommerce/' . $result, file_get_contents($url_1));
                $data_1 = [
                    'featured_images' => $file_name_2,
                ];
                $this->db->where('id', $product_id);
                $this->db->update('ecommerce_product', $data_1);
            }
        }
    }

    function img_thumb($type, $id, $ext, $width = '400', $height = '400') {
        ...
        $config1['source_image'] = 'upload/ecommerce/test/' . $type . '_image/' . $type . '_' . $id . $ext;
    ...
    function file_exist_ret2($type, $id, $ib, $ext = '.jpg') {
        if (file_exists(FCPATH . 'upload/ecommerce/test/' . $type . '_image/' . $type . '_' . $id . '_' . $ib . $ext)) {
            $ib = $ib + 1;
    ...
    public function change_order_status() {
        ...
        $update_data = array("status" => $payment_status, "status_changed_at" => date("Y-m-d H:i:s"));
        $update_data['status_changed_note'] = $status_changed_note;
        $update_data['shipped_through'] = $this->input->post("shipped_through", true);
  ...
  "weight": 2.5
    ...
    function send_oder_notification() {
        $user_id = $this->input->post("user_id");
        $headings = $this->input->post("headings");
        $query = $this->db->query("select * from order_notification where user_id='" . $this->session->userdata('user_id') . "'")->result();
        foreach ($query as $value) {
            $to = $value->player_id;
            $content = array(
                "en" => "You have received new order!!"
            );
            $headings = array(
                "en" => "$headings"
            );
            ...
            $fields = array(
                'app_id' => "3aec7a55-f7ce-4581-b5c9-325096def6c9",
            ...
            //print("\nJSON sent:\n");
            //$rest_api='NjVlNmQ1ZjMtMTkxYy00ZTI2LWFmODktMmQ1NTM4YWJiYWQw';

            $ch = curl_init();
            curl_setopt($ch, CURLOPT_URL, "https://onesignal.com/api/v1/notifications");
            curl_setopt($ch, CURLOPT_HTTPHEADER, array('Content-Type: application/json; charset=utf-8',
                'Authorization: Basic ZjhiMzIyOTItZDRhZS00NjU0LWE5OWYtYzJhMGI5NDk4ZGUw'));
    ...
    public function sendMobileNotification($cart_id, $amount, $type) {

        if ($type == 'new_order') {
            $user_id = $this->db->query("SELECT user_id FROM ecommerce_cart WHERE id='" . $cart_id . "'")->row()->user_id;

            $fcm_token = $this->db->query("SELECT fcm_token FROM users WHERE id='" . $user_id . "'")->row()->fcm_token;

            $get_data = $this->db->query("SELECT ecommerce_cart.id,ecommerce_cart.buyer_first_name,ecommerce_cart.buyer_last_name,ecommerce_cart.is_notification_view,ecommerce_cart_item.cart_id,ecommerce_cart_item.product_id,ecommerce_product.product_name,ecommerce_cart_item.quantity,ecommerce_cart.buyer_address FROM `ecommerce_cart` INNER JOIN ecommerce_cart_item ON ecommerce_cart_item.cart_id=ecommerce_cart.id INNER JOIN ecommerce_product ON ecommerce_cart_item.product_id=ecommerce_product.id WHERE ecommerce_cart.user_id='" . $user_id . "' ORDER BY ecommerce_cart.id DESC LIMIT 1")->result();
            ...
            $user_id = $this->db->query("SELECT user_id FROM ecommerce_cart WHERE id='" . $cart_id . "'")->row()->user_id;
            $device_token = $this->db->query("SELECT fcm_token FROM users WHERE id='" . $user_id . "'")->row()->fcm_token;
            $user_data = array(
                "notification_type" => "payment",
            ...
            $data = array(
                'title' => 'Payment Update',
                'body' => "you have received one thousand $amount",
            );
            $temp['notification'] = $data;
            $temp_1['data'] = $user_data;
        ...
        $message = json_encode($final_data);
...
//echo $response;
    ...
    public function get_ecommerce_sub_category() {
        $category_id = strip_tags($this->input->post("category_id", true));
        $this->db->select("*");
        $this->db->from('ecommerce_sub_category');
        $this->db->where('category_id', $category_id);
        $query = $this->db->get()->result();
        echo json_encode($query);
    }
    ...
    public function catalog() {
        $data['body'] = 'ecommerce/catalog';
        $data['page_title'] = $this->lang->line('catalog');
        $data['catalog'] = $this->db->query("SELECT * FROM `catalog` WHERE is_deleted=0")->result();
        $data['category'] = $this->db->query("SELECT id,name FROM `category` WHERE is_deleted=0")->result();
        $this->_viewcontroller($data);
    }

    public function category($param = '', $param2 = '') {
        if ($param == 'list') {
            $data['body'] = 'ecommerce/catalog_category_list';
            $data['page_title'] = $this->lang->line('category');
            $this->_viewcontroller($data);
        }
    }

    public function sub_category($param = '', $param2 = '') {
        if ($param == 'list') {
            $data['body'] = 'ecommerce/catalog_sub_category_list';
            $data['page_title'] = $this->lang->line('Sub category');
            $data['category'] = $this->db->query("SELECT id,name FROM `category` WHERE is_deleted=0")->result();
            $this->_viewcontroller($data);
        }
    }

    public function category_list_data1() {
        $search_value = $_POST['search']['value'];
        $display_columns = array("#", 'CHECKBOX', 'serial', 'thumbnail', 'name', 'status', 'actions', 'updated_at',);
        ...
        $order = isset($_POST['order'][0]['dir']) ? strval($_POST['order'][0]['dir']) : 'asc';
        $order_by = $sort . " " . $order;
        $info = $this->db->query("select * from category")->result_array();
        $total_result = $this->db->query("select * from category")->num_rows();
    ...
    public function ajax_create_new_category1() {
        $this->ajax_check();
        $result = array('status' => '0', 'message' => $this->lang->line("Something went wrong, please try once again."));
        ...
        if ($_POST) {
            ...
            if ($this->basic->insert_data("category", $inserted_data)) {
    ...
    public function catalog_list_data() {
        $search_value = $_POST['search']['value'];
        $display_columns = array("#", 'CHECKBOX', 'serial', 'thumbnail', 'name', 'status', 'actions', 'updated_at',);
        $search_columns = array('name');
...
//        $where_custom = '';
//        $where_custom = "category.user_id = " . $this->user_id . " AND ecommerce_category.store_id = " . $this->session->userdata("ecommerce_selected_store");
//        if ($search_value != '') {
//            foreach ($search_columns as $key => $value)
//                $temp[] = $value . " LIKE " . "'%$search_value%'";
//            $imp = implode(" OR ", $temp);
...
//        $info = $this->basic->get_data($table, $where = '', $select, $limit, $start, $order_by, $group_by = '');
        $info = $this->db->query("select * from catalog")->result_array();

        //$this->db->where($where_custom);
//        $total_rows_array = $this->basic->count_row($table, $where = '', $count = "ecommerce_category.id", $group_by = '');
        $total_result = $this->db->query("select * from catalog")->num_rows();
        //$total_result = $total_rows_array[0]['total_rows'];

        foreach ($info as $key => $value) {
//            $info[$key]['actions'] = "<div style='min-width:100px'><a href='#' title='" . $this->lang->line("Edit") . "' data-toggle='tooltip' class='btn btn-circle btn-outline-warning edit_row' table_id='" . $info[$key]['id'] . "'><i class='fa fa-edit'></i></a>&nbsp;&nbsp;";

            $info[$key]['actions'] = "<div style='min-width:100px'><a href='#' title='" . $this->lang->line("Delete") . "' data-toggle='tooltip' class='btn btn-circle btn-outline-danger delete_row' table_id='" . $info[$key]['id'] . "'><i class='fa fa-trash-alt'></i></a></div>
    <script>$('[data-toggle=\"tooltip\"]').tooltip();</script>";
    ...
    public function create_catalog_product() {
        $this->ajax_check();
        $result = array('status' => '0', 'message' => $this->lang->line("Something went wrong, please try once again."));
        ...
        if ($_POST) {
            ...
            $category_id = strip_tags($this->input->post("category_id", true));
            $sub_category_id = strip_tags($this->input->post("sub_category_id", true));
            $thumbnail = $this->input->post('thumbnail', true);
            ...
            if ($this->basic->insert_data("catalog", $inserted_data)) {

                $result['status'] = "1";
                $result['message'] = $this->lang->line("Catalog has been added successfully.");
            }
    ...
    public function delete_catalog() {
        ...
        if ($this->basic->delete_data("catalog", array("id" => $table_id)))
            echo json_encode(array('message' => $this->lang->line("Category has been deleted successfully."), 'status' => '1'));
        else
            echo json_encode($result);
    }

    public function upload_catalog_product_thumb() {
        // Kicks out if not a ajax request
        $this->ajax_check();
        ...
        if (isset($_FILES['file'])) {
            ...
            if (is_uploaded_file($tmp_file)) {
                ...
                $filename = implode('.', $post_fileName_array);
                $filename = strtolower(strip_tags(str_replace(' ', '-', $filename)));
                $filename = "product_" . time() . substr(uniqid(mt_rand(), true), 0, 6) . '.' . $ext;
    ...
    public function upload_category_thumb1() {
        // Kicks out if not a ajax request
        $this->ajax_check();
        ...
        if (isset($_FILES['file'])) {
            ...
            if (is_uploaded_file($tmp_file)) {
                ...
                $filename = implode('.', $post_fileName_array);
                $filename = strtolower(strip_tags(str_replace(' ', '-', $filename)));
                $filename = "category_" . time() . substr(uniqid(mt_rand(), true), 0, 6) . '.' . $ext;
    ...
    public function delete_category1() {
        $this->ajax_check();
        $table_id = $this->input->post("table_id");
        $result = array('status' => '0', 'message' => $this->lang->line("Something went wrong, please try once again."));
        if ($table_id == "0" || $table_id == "") {
        ...
        if ($this->basic->delete_data("category", array("id" => $table_id)))
            echo json_encode(array('message' => $this->lang->line("Category has been deleted successfully."), 'status' => '1'));
        else
            echo json_encode($result);
    }

    public function ajax_update_catalog_category() {
        $this->ajax_check();
        $result = array('status' => '0', 'message' => $this->lang->line("Something went wrong, please try once again."));
        ...
        if ($_POST) {
            ...
            if ($this->basic->update_data("category", array("id" => $table_id), $updated_data)) {
                $result['status'] = "1";
                $result['message'] = $this->lang->line("Category has been updated successfully.");
            }
    ...
    public function ajax_get_catalog_category_update_info() {
        ...
        $details = $this->basic->get_data("category", array('where' => array('id' => $table_id)));
        //print_r($details);
        $selected = ($details[0]['status'] == '1') ? 'checked' : '';
  ...
  <div class="form-group">
  <label for="name">' . $this->lang->line("Store") . ' *</label>
  ' . form_dropdown('', $store_list, $details[0], ' style="width:100%;" disabled class="form-control seelct"') . '
  </div>
  </div>
  <div class="col-12">
  <div class="form-group">
  <label>' . $this->lang->line('Category Name') . ' *</label>
  <input type="text" class="form-control" name="category_name2" id="category_name2" value="' . $details[0]['name'] . '">
  </div>
  </div>
  <div class="col-12">
  <div class="form-group">
  ...
  $(document).ready(function() {
    ...
    public function catalog_sub_category_list_data() {
        $search_value = $_POST['search']['value'];
        $display_columns = array("#", 'CHECKBOX', 'serial', 'thumbnail', 'name', 'category_name', 'status', 'actions', 'updated_at',);
        $search_columns = array('name');
        $page = isset($_POST['page']) ? intval($_POST['page']) : 1;
        $start = isset($_POST['start']) ? intval($_POST['start']) : 0;
        $limit = isset($_POST['length']) ? intval($_POST['length']) : 10;
        ...
        $order = isset($_POST['order'][0]['dir']) ? strval($_POST['order'][0]['dir']) : 'asc';
        $order_by = $sort . " " . $order;
        $info = $this->db->query("select * from sub_category")->result_array();
        $total_result = $this->db->query("select * from sub_category")->num_rows();
        foreach ($info as $key => $value) {
            $category_id = $info[$key]['category_id'];
            $category_name = $this->db->query("select name from category where id='" . $category_id . "'")->row()->name;
            $info[$key]['actions'] = "<div style='min-width:100px'><a href='#' title='" . $this->lang->line("Edit") . "' data-toggle='tooltip' class='btn btn-circle btn-outline-warning edit_row' table_id='" . $info[$key]['id'] . "'><i class='fa fa-edit'></i></a>&nbsp;&nbsp;";
    ...
    public function create_new_sub_category() {
        $this->ajax_check();
        $result = array('status' => '0', 'message' => $this->lang->line("Something went wrong, please try once again."));

        if ($_POST) {
            $category_id = strip_tags($this->input->post("category_id", true));
            $sub_category = strip_tags($this->input->post("sub_category_name", true));
            $thumbnail = $this->input->post('thumbnail', true);
            ...
            if ($this->basic->insert_data("sub_category", $inserted_data)) {
                $result['status'] = "1";
                $result['message'] = $this->lang->line("Sub Category has been added successfully.");
            }
    ...
    public function upload_sub_category_thumb() {
        // Kicks out if not a ajax request
        $this->ajax_check();
        ...
        if (isset($_FILES['file'])) {
            ...
            if (is_uploaded_file($tmp_file)) {
                ...
                // Sets filename to session
                $this->session->set_userdata('Sub_category_thumb_uploaded_file', $filename);
    ...
    public function delete_catalog_sub_category() {
        $this->ajax_check();
        $table_id = $this->input->post("table_id");
        $result = array('status' => '0', 'message' => $this->lang->line("Something went wrong, please try once again."));
        if ($table_id == "0" || $table_id == "") {
        ...
        if ($this->basic->delete_data("sub_category", array("id" => $table_id)))
            echo json_encode(array('message' => $this->lang->line("Sub Category has been deleted successfully."), 'status' => '1'));
        else
            echo json_encode($result);
    }

    public function ajax_get_catalog_sub_category_update_info() {
        ...
        $details = $this->basic->get_data("sub_category", array('where' => array('id' => $table_id)));
        $selected = ($details[0]['status'] == '1') ? 'checked' : '';
        $category = $this->db->query("SELECT id,name FROM `category` WHERE is_deleted=0")->result();
        $form = '<div class="row">
  <div class="col-12">
  <form action="#" enctype="multipart/form-data" id="row_update_form" method="post">
  <input type="hidden" name="table_id" value="' . $table_id . '">
...
<div class="col-12">
        ...
        foreach ($category as $cat) {
            if ($cat->id == $details[0]['category_id']) {
                ...
                $form .= '<option selected value="' . $cat->id . '">' . $cat->name . '</option>';
            } else {
                $form .= '<option value="' . $cat->id . '">' . $cat->name . '</option>';
            }
        }
  ...
  <div class="form-group">
  <label>' . $this->lang->line('Sub Category Name') . ' *</label>
  <input type="text" class="form-control" name="sub_category" id="sub_category" value="' . $details[0]['name'] . '">
  </div>
  </div>
  <div class="col-12">
  <div class="form-group">
  ...
  $(document).ready(function() {
    ...
    Dropzone.autoDiscover = false;
    $('#thumb-dropzone2').dropzone({
      url: '" . base_url('ecommerce/upload_sub_category_thumb') . "',
      maxFilesize:.5,
      uploadMultiple:false,
      paramName:'file',
      createImageThumbnails:true,
            ...
            function delete_uploaded_file2(filename) {
              if('' !== filename) {
                $.ajax({
                  ...
                  dataType: 'JSON',
                  data: { filename },
                  url: '" . base_url('ecommerce/delete_sub_category_thumb') . "',
                  success: function(data) {
                    $('#uploaded-file2').val('');
                  }
                  });
    ...
    public function delete_sub_category_thumb() {
        // Kicks out if not a ajax request
        $this->ajax_check();
        ...
        // Grabs filename
        $filename = (string) $this->input->post('filename');
        $session_filename = $this->session->userdata('Sub category_thumb_uploaded_file');
        if ($filename !== $session_filename) {
            exit;
        }
        ...
        if (file_exists($filepath)) {
            ...
            // Deletes file from session
            $this->session->unset_userdata('sub_category_thumb_uploaded_file');
    ...
    public function ajax_update_catalog_sub_category() {
        $this->ajax_check();
        $result = array('status' => '0', 'message' => $this->lang->line("Something went wrong, please try once again."));
        ...
        if ($_POST) {
            $table_id = $this->input->post("table_id", true);
            $category_id = strip_tags($this->input->post("category_id", true));
            $sub_category = strip_tags($this->input->post("sub_category", true));
            $thumbnail = $this->input->post("thumbnail2", true);
            ...
            if ($this->basic->update_data("sub_category", array("id" => $table_id), $updated_data)) {
                $result['status'] = "1";
                $result['message'] = $this->lang->line("Sub Category has been updated successfully.");
            }
    ...
    public function get_category_wise_product() {
        ...
        $query = $this->db->query("select * from catalog where sub_cat_id='" . $sub_category_id . "'")->result();
        $count = 1;
        foreach ($query as $row) {
            $product_img=$row->thumbnail;
            ?>
            <div class="row catalog-section">
                <div class="col-2">
                    <div class="form-group catalog-image">
                    <?php
                    if ($row->thumbnail == '')
              echo  $url = base_url('assets/img/icon/rocket.png');
            else
                $url = base_url('upload/ecommerce/' . $row->thumbnail);
           echo $row->thumbnail = "<a  target='_BLANK' href='" . $url . "'><img class='rounded-circle' style='height:50px;width:50px;border-radius:4px;border:1px solid #eee;padding:2px;' src='" . $url . "'></a>";
                    ?>
                    </div>
                </div>
                <div class="col-4">
                    <div class="form-group">
                        ...
                        <!-- <input type="text" class="form-control" id="product_name_<?= $count ?>" value="<?= $row->name ?>" readonly>-->
                        <input type="hidden" name="product_name[]" value="<?= $row->name ?>">
                        <input type="hidden" name="thumbnail[]" value="<?= $product_img ?>">
                    </div>
                </div>
                <div class="col-2">
                    <div class="form-group">
                        <!-- <label> <?php echo $this->lang->line("Price") ?></label> -->
                        <input type="text" name="price[]" class="form-control" placeholder="Price" id="price_<?= $count ?>" value="<?= $row->price ?>">
                    </div>
                </div>
                <div class="col-2">
                    <div class="form-group">
                        <!-- <label> <?php echo $this->lang->line("qty") ?></label> -->
                        <input type="text" name="qty[]" class="form-control" placeholder="Stock" id="qty_<?= $count ?>">
                    </div>
                </div>
                <div class="col-2">
                    <div class="form-group">
                        <!-- <label> <?php echo $this->lang->line("Check") ?></label> -->
                        <input type="checkbox" name="is_checked[]" id="is_checked_<?= $count ?>" value="yes">
                    </div>
                </div>
            </div>
            <?php
    ...
    public function add_catalog_product() {
        ...
        if ($_POST) {
            ...
            $is_checked = count($this->input->post("is_checked"));


            //$category_name = $this->db->query("select * from category where id='" . $category_id . "'")->row_array();
          //  $sub_category_name = $this->db->query("select * from sub_category where id='" . $sub_category_id . "'")->row_array();
              $q = $this->db->query("SELECT * FROM ecommerce_category WHERE catelog_cat_id='" . $category_id . "' and catelog_subcat_id='".$sub_category_id."' and user_id ='"$user_id"'");
            if ($q->num_rows() > 0) {
              $query = $q->row();
              $cat_id = $query->id;
            }else{
              $category_name = $this->db->query("select * from sub_category where id='" . $sub_category_id . "'")->row_array();
            ...
            $sq = $this->db->query("SELECT * FROM ecommerce_sub_category WHERE catelog_cat_id='" . $category_id . "' and catelog_subcat_id='".$sub_id."' and user_id ='"$user_id"'");
            if ($sq->num_rows() > 0) {
              $squery = $sq->row();
              $subcat_id = $squery->id;
            }else{
             ...
             $ecommerce_sub_category=array('user_id'=> $user_id,
                ...
                'category_id'=> $cat_id,
                 'catelog_cat_id'=>$category_id,
                'catelog_subcat_id'=>$sub_category_id,
    ...
    public function delete_catalog_thumb() {
        // Kicks out if not a ajax request
        $this->ajax_check();
        ...
        // Grabs filename
        $filename = (string) $this->input->post('filename');
        $session_filename = $this->session->userdata('catalog_thumb_uploaded_file');
        if ($filename !== $session_filename) {
            exit;
        }
        ...
        if (file_exists($filepath)) {
            ...
            // Deletes file from session
            $this->session->unset_userdata('catalog_thumb_uploaded_file');
    ...
    public function download_invoice($id = 0) {
        error_reporting(0);
        $store_data_temp = $this->basic->get_data("ecommerce_cart", array("where" => array("id" => $id)), "store_id");
        $store_id = isset($store_data_temp[0]['store_id']) ? $store_data_temp[0]['store_id'] : "0";

        $page_data['order_details'] = $this->db->query("SELECT * FROM `ecommerce_cart` WHERE ecommerce_cart.id=$id AND store_id='" . $store_id . "'")->row();
        $page_data['store'] = $this->db->query("SELECT * FROM `ecommerce_store` WHERE id='" . $store_id . "'")->row();
        $page_data['order'] = $this->db->query("SELECT ecommerce_cart_item.store_id,ecommerce_cart_item.cart_id,ecommerce_cart_item.product_id,ecommerce_cart_item.unit_price,ecommerce_cart_item.coupon_info,ecommerce_cart_item.quantity,ecommerce_cart_item.attribute_info,ecommerce_product.id,ecommerce_product.product_name,ecommerce_product.product_description,ecommerce_product.thumbnail FROM `ecommerce_cart_item` INNER JOIN ecommerce_product ON ecommerce_product.id=ecommerce_cart_item.product_id WHERE ecommerce_cart_item.cart_id=$id")->result();
        // $html = $this->load->view('ecommerce/invoice_download', $page_data,true);
        $filename = time() . ".pdf";
        $this->load->view('ecommerce/invoice_download',$page_data);
        $html = $this->output->get_output();
        // Load pdf library
        $css_file = base_url() . 'assets/site_new/css/bootstrap.min.css';
        ...
        file_put_contents('upload/invoice/' . $filename, $output);
        $this->load->helper('download');
        $data = file_get_contents(base_url('upload/invoice/' . $filename));
        force_download($filename, $data);
        // file_put_contents(base_url('upload/invoice/' . $filename, $output));
    }
