...
class Flow_builder{
	...
	public function get_new_sequence_info($new_sequence_node_id,$input_type){
		...
		$new_squence_info['xitFbUniqueSequenceId']= isset($this->flow_array['nodes'][$new_sequence_node_id]['data']['xitFbUniqueSequenceId']) ? $this->flow_array['nodes'][$new_sequence_node_id]['data']['xitFbUniqueSequenceId'] : "";
	...
	public function get_user_input_flow_info($node_id){
		...
		$user_input_flow_information['unique_id']= isset($this->flow_array['nodes'][$node_id]['data']['userInputFlowId']) ? $this->flow_array['nodes'][$node_id]['data']['userInputFlowId'] : "";
		...
		while($question_node_id!=""){
			...
			$user_input_flow_information['questions'][$no_question]['unique_id'] = isset($this->flow_array['nodes'][$question_node_id]['data']['userInputFlowSingleId']) ? $this->flow_array['nodes'][$question_node_id]['data']['userInputFlowSingleId'] : "";
			...
			if($question_node_type=='newPostbackInput'){
	...
	public function extract_json($flow_array){
		...
		for($i=0;$i<$count;$i++){
			...
			if ($output_type=="referenceOutput"){
				...
				$bot_settings_array[$no_of_entry]['xitFbUniqueId']= isset($this->flow_array['nodes'][$index_id]['data']['xitFbUniqueId']) ? $this->flow_array['nodes'][$index_id]['data']['xitFbUniqueId'] : "";

				if($action_button_type=="messenger_engagement_plugin"){
					$button_info = [];
					$bot_settings_array[$no_of_entry]['messengerEngagementId']= isset($this->flow_array['nodes'][$action_button_node_id]['data']['messengerEngagementId']) ? $this->flow_array['nodes'][$action_button_node_id]['data']['messengerEngagementId'] : "";
